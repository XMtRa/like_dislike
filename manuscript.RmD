---
title             : "Likes or Dislikes, Gratifications or Concerns? Analyzing Popularity Cues and Privacy Calculus in Online Communication"

shorttitle        : "Popularity cues & privacy calculus"

abstract: |
    According to the privacy calculus model, online communication is mainly determined by the two factors of privacy concerns and expected benefits. This paper analyzes how the privacy calculus is affected by popularity cues, namely like and dislike buttons. In addition, the privacy calculus is extended by elaborating on the process of comparing benefits and drawbacks, termed privacy deliberation, and by investigating the role of trust and self-efficacy. To this end, a preregistered one-week field experiment with 590 participants was conducted. Participants were randomly distributed to three different websites, on which they discussed a current political topic. The websites featured either (a) like buttons, (b) both like and dislike buttons, or (c) no like or dislike buttons. The exploratory analyses revealed, contrary to what was expected, that implementing both like and dislike buttons decreased online communication. As predicted by the privacy calculus model, expected benefits and perceived privacy concerns substantially affected communcation. Actively deliberating about privacy by comparing benefits and drawbacks decreased communication, while experiencing self-efficacy and trust strongly increased communication. 
...
Research is therefore increasingly building on the privacy calculus model, which states that communication online can be explained successfully by means of perceived risks and expected benefits [@bolUnderstandingEffectsPersonalization2018b; @krasnovaOnlineSocialNetworks2010; @meierPrivacyCalculusRevisited2022].
...
First, we still know little about how the privacy calculus and online communication can be affected by external influences [@bazarovaIntegrationIndividualisticNetworked2020; @braunlichLinkingLooseEnds2020].
For example, does the existence of popularity cues such as like and dislike buttons affect the privacy calculus and online communication?
...
Can the privacy calculus be replicated in an authentic and more long-term setting using actual behavioral data?
...
which I will do in this study by elaborating on the privacy deliberation process. 
Fourth, the privacy calculus model is criticized for over-emphasizing rationality [@knijnenburgDeathPrivacyCalculus2017].
Hence, I extend the privacy calculus in analyzing how trust and self-efficacy, two more psychological variables, affect communication.
...
When are people willing to engage in a conversation?
<!-- Do they express their opinion?  -->
It builds on the calculus of behavior [@lauferPrivacyConceptSocial1977], which states that people are weighing risks and benefits before communicating.
...
People are happy to trade in parts of their privacy, when they can get something more valuable in return [@lauferPrivacyConceptSocial1977].
...
Instead, we need to analyze if and how much people actively deliberate about their privacy, and whether doing so influences their decision to communicate.
...
How could deliberating on one's privacy affect communication?
...
In light of the benefits mentioned above, a person might conclude that communication is beneficial, thereby legitimating communication [@krasnovaOnlineSocialNetworks2010]. 
...
But although human behavior is partly irrational, bounded rationality does not state that it is completely irrational [@gigerenzerBoundedRationalityAdaptive2002].
...
Factors less focused on rationality, but more on psychosocial aspects.
...
In online contexts, there are different _targets_ of trust, including (a) the information system, (b) the provider, (c) the Internet, and (d) the community of other users [@sollnerWhyDifferentTrust2016]. 
...
First, it makes sense not to focus on specific features of particular websites, which change and become obsolete [@foxDistinguishingTechnologiesSocial2017]. 
...
Second, _actually receiving_ likes or dislikes might additionally affect subsequent behavior, thereby reinforcing the process.

Likes are positive and represent the positivity bias of social media [@schreursExposurePositivityBias2022]. 
...
Dislike represent a punishment, introducing a _loss frame_. 
<!-- Although most communication emphasizes positive aspects, the Internet is also replete with spite, envy, and arguments. -->
Websites featuring both like _and_ dislike buttons should be more ambivalent compared to websites without popularity cues.
...
In some cases, the pre-registered approach needed to be changed (see [companion website](https://XMtRa.github.io/like_dislike)). 
...
The first experimental group used a website that included like buttons buttons; the second experimental group used an identical website including both like and dislike buttons; and the control group used an identical website without like and dislike buttons. 
...
The questionnaire asked participants about their sociodemographic background, (b) provided more details about the study, and (c) included a registration link for the website, which was described as "participation platform".
...
Participants had the chance to use the website and to provide comments over the course of one week.
...
For a screenshot of the landing page and an example of comments that were written, see Figure \@ref(fig:website).
...
The power analysis was based on a smallest effect size of interest approach [SESOI, @lakensEquivalenceTestingPsychological2018]. 
...
Sensitivity analyses showed that the current study could still make reliable results (i.e., power = 95%) for effects at least as large as _r_ = `r r_sensitive %>% my_round("std")`.
...
For the exact wording of all items and their individual distributions, see [companion website](https://XMtRa.github.io/like_dislike).
...
Although it is possible to analyze non-normal data with structural equation modeling, it is recommended to use analyses that model the variables' distribution, which can be done with Bayesian hurdle models [@mcelreathStatisticalRethinkingBayesian2016].
...
For more information, see online supplementary material.
...
For example, in the bivariate analysis trust had a _positive_ relation with communication, whereas in the multiple regression the effect was _negative_---which should make us wary.
...
If participants expected more gratifications from participation, they communicated more: 
If their expected gratifications increased by one point, on average they also wrote `r tab_slopes %>% filter(Outcome == "words" & Predictor == "Expected gratifications") %>%  select(Estimate) %>% round(0)` more words (95% CI: `r tab_slopes %>% filter(Outcome == "words" & Predictor == "Expected gratifications") %>%  select(LL) %>% round(0)`, `r tab_slopes %>% filter(Outcome == "words" & Predictor == "Expected gratifications") %>%  select(UL) %>% round(0)`). 
If participants were more concerned about their privacy, they communicated less: 
With each one-point increase in privacy concerns, on average they wrote `r tab_slopes %>% filter(Outcome == "words" & Predictor == "Privacy concerns") %>%  select(Estimate) %>% round(0) %>% abs()` fewer words (95% CI: `r tab_slopes %>% filter(Outcome == "words" & Predictor == "Privacy concerns") %>%  select(LL) %>% round(0)`, `r tab_slopes %>% filter(Outcome == "words" & Predictor == "Privacy concerns") %>%  select(UL) %>% round(0)`).
If participants felt more self-efficacious, they also communicated much more:
If their self-efficacy increased by one point, on average they wrote `r tab_slopes %>% filter(Outcome == "words" & Predictor == "Self-efficacy") %>%  select(Estimate) %>% round(0)` more words (95% CI: `r tab_slopes %>% filter(Outcome == "words" & Predictor == "Self-efficacy") %>%  select(LL) %>% round(0)`, `r tab_slopes %>% filter(Outcome == "words" & Predictor == "Self-efficacy") %>%  select(UL) %>% round(0)`).
The relationship was curvilinear, almost exponential: 
Whereas a change in self-efficacy from 1 to 2 only led to an "increase" of `r effects_wrds_selfeff_12 %>% round(0) %>% unlist %>% english()` words, a change from 6 to 7 led to an increase of `r effects_wrds_selfeff_67 %>% round(0)` words.
And if participants experienced more trust in the website, provider, and other users, they also communicated much more:
If their trust increased by one point, on average they wrote `r tab_slopes %>% filter(Outcome == "words" & Predictor == "Trust") %>%  select(Estimate) %>% round(0)` more words (95% CI: `r tab_slopes %>% filter(Outcome == "words" & Predictor == "Trust") %>%  select(LL) %>% round(0)`, `r tab_slopes %>% filter(Outcome == "words" & Predictor == "Trust") %>%  select(UL) %>% round(0)`).
...
Contrary to what I expected, implementing both popularity cues did not increase but rather decreased communication.
...
Results showed that this was not the case (see online companion material),
...
The preregistered analyses using structural equation modeling with multiple predictors were hence subpar.

To address these shortcoming, in several exploratory analyses I conducted Bayesian hurdle-gamma models (see Data analysis).
...
People who were more concerned about their privacy wrote significantly less words. 
...
To illustrate, if people reported no self-efficacy, they wrote only `r wrds_selfeff_1` words on average.
...
Although more rational aspects such as costs and benefits influence behavior, behavior is also determined by more psychosocial variables such as trust and efficacy.
...
Privacy deliberation and privacy concerns were strongly correlated (_r_ = .61), showing comparable correlations with other variables. 
...
At this point, it is still unknown if thinking about privacy increases concerns or, conversely, if growing more concerned about privacy encourages us to deliberate more carefully. 
...
Although we would expect that like buttons, being positive feedback cues, would increase communication, it also seems plausible that they can decrease communication.
...
Although like buttons are commonplace in social media, the findings suggests that in specific contexts they inhibit communication.
...
Whereas the effects of the popularity cues on all variables can be interpreted from a causal perspective (but see below), more caution is needed when regarding the effects of the privacy calculus variables on communication.
Although the effects were controlled for age, gender, and education, other variables not included here might exist that potentially bias the causal estimates presented here [@coenenIndirectEffectOmitted2022].
...
But, similar to everyday offline contexts, users are also affected by the affordances of their environment, often acting implicitly. 
