...
abstract: |
    The privacy calculus model states that people weigh risks and benefits before communicating online. This paper analyzes three open questions. How robust is the privacy calculus, can it be confirmed with actual behavior in realistic online setting? Do we need to extend the model by integrating more socio-psychological measures including trust, self-efficacy, and privacy deliberation? Can the privacy calculus be be manipulated by external factors such as like and dislike buttons? A preregistered one-week field experiment with 590 participants was conducted. Participants discussed a current political topic on an online website, which included either (a) like buttons, (b) both like and dislike buttons, or (c) no like or dislike buttons. The privacy calculus model was confirmed: Benefits and concerns affected communication. Deliberating about privacy by comparing benefits and drawbacks decreased communication, while experiencing self-efficacy and trust increased communication. Implementing both like and dislike buttons reduced online communication by 44%. 
...
Research is therefore increasingly building on the privacy calculus model, which states that communication online can be explained by perceived risks and expected benefits [@bolUnderstandingEffectsPersonalization2018b; @krasnovaOnlineSocialNetworks2010; @meierPrivacyCalculusRevisited2022].
...
Second, the privacy calculus model is criticized for neglecting the actual process of weighing pros and for over-emphasizing rationality  [@knijnenburgDeathPrivacyCalculus2017].
...
Instead, we need to find out if people deliberately compared costs and benefits before communicating online. 
Therefore, I will also analyze the explicit process of comparing pros and cons, by elaborating on what I will call the the privacy deliberation process. 
Next, focusing exclusively on costs and benefits does not sufficiently represent how and why users engage online.
Hence, I extend the privacy calculus in analyzing how online communication is affected by trust and self-efficacy, two variables less focused on rationality and more on socio-psychological aspects [@metzgerPrivacyTrustDisclosure2004].
...
Online contexts are optimized to elicit as much interaction as possible, for example via low threshold communication features such as likes, shares, replies, or reactions  [@carrPredictingThresholdPerceived2018]. 
Implicit and explicit cues on how others use a website can increase self-disclosure [@spottswoodShouldShareThat2017; @treptePrivacyCalculusContextualized2020].
<!-- The third and last question of this study is therefore as follows:  -->
...
And can the privacy calculus be manipulated by popularity cues?
...
Although privacy calculus implies that people explicitly compare benefits and drawbacks before communicating online, prior research has neglected this aspect [@knijnenburgDeathPrivacyCalculus2017].
...
In light of the benefits mentioned above, it can make much sense to conclude that participation is beneficial, thereby fostering communication [@krasnovaOnlineSocialNetworks2010]?
...
Prior research suggests that two factors are most important to understand online communication: self-efficacy and trust [correaParticipationDivideOnline2010; hossainICTEnabledVirtual2004; @metzgerPrivacyTrustDisclosure2004].
...
In all situations where people lack experience, control, or competence, a central variable to understand behavior is _trust_ [@gefenTrustTAMOnline2003].
Especially in online contexts, trust plays a key role [@metzgerPrivacyTrustDisclosure2004], as users often cannot control the environment or the way their information is handled [@acquistiSecretsLikesDrive2020; @braunlichLinkingLooseEnds2020].
...
How easily can the calculus be affected externally?
One of the central tools to afford and govern online behavior are so-called popularity cues such as like and dislike buttons [@stsiampkouskayaNotExperimentalStudy2023].
...
In analyzing this question, it makes sense to analyze the cues' underlying _affordances_ [@ellisonSocialNetworkSite2015; @foxDistinguishingTechnologiesSocial2017].
...
Popularity cues such as like and dislike buttons are understood as "paralinguistic digital affordances" [@carrPredictingThresholdPerceived2018, p. 142].
...
To illustrate, likes are affirmative and represent the positivity bias of social media [@schreursExposurePositivityBias2022]. 
...
The mean lay on the scale's neutral midpoint (_m_ = `r facval_tab["Privacy deliberation", "m"]`). For an interpretation, see below.
...
To address these shortcomings, in several exploratory analyses I conducted Bayesian hurdle-gamma models (see section Data analysis).
...
In addition, not to reveal the study intention the self-reported measures were collected after the field phase. 
<!-- , in which the dependent variable was measured. -->
